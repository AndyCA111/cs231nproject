...
def create_dataset(dataset_name, img_size=176, T=None, prompt='a mri brain image', deterministic=False, num_workers=1, return_dataset=False, ifexample = False,):
    data_path = video_data_paths_dict[dataset_name]
    T = default_T_dict[dataset_name] if T is None else T
    if dataset_name == "MRI":
        dataset = MRIDataset(train=True, imgsize=img_size, path=data_path, shard=0, num_shards=0, T=T, prompt=prompt, ifexample = ifexample,)
    else:
...
class MRIDataset(BaseDataset):
    def __init__(self, path, train=None, shard=None, num_shards=None, T=None, resize=False, imgsize =176, prompt ='a brain image',ifexample=False,):
    ...
    def loaditem(self, path):
        ...
        if self.imgsize == 176:
            img2 = np.ones((144, 176, 144))*(-1)
            # img2 = np.ones((144, 192, 144))*(-1)
            img2[2:2+138,:,2:2+138] = data
            img = np.transpose(img2,(2,1,0))
        if self.imgsize == 192:
            img2 = np.ones((144, 192, 144))*(-1)
            # img2 = np.ones((144, 192, 144))*(-1)
            img2[1:1+138,8:8+176,1:1+138] = data
            img = np.transpose(img2,(2,1,0))
        ...
        if self.imgsize == 176:
            assert data.shape == (3, 144, 176, 144)
        if self.imgsize == 192:
            assert data.shape == (3, 144, 192, 144)   
...
def get_data_loaders(
    ...
    dataset = create_dataset(dataset_name, img_size=img_size, prompt=prompt, ifexample = ifexample)
...
if __name__ == "__main__":
    a, b = get_data_loaders("MRI", 176, 1, ifreturn_loader=True, ifexample = False,)
    print(a)
