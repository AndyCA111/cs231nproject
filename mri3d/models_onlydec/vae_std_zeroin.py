...
from mri3d.models_onlydec.vae_blocks_zeroin import (
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.conv_in = torch.nn.Conv2d(in_channels, block_out_channels[0], kernel_size=3, padding=1)
...
class DiagonalGaussianDistribution(object):
    ...
    def sample(self, auto_reshape=True, generator: Optional[torch.Generator] = None) -> torch.FloatTensor:
        ...
        assert x.dim() == 4
        if auto_reshape:
        #'(b,f),c,h',w' -> b c f h' w' but we assume batch = 1
    ...
    def mode(self, auto_reshape=True):
        x = self.mean
        if auto_reshape:
        #'(b,c),c,h',w' -> b c f h' w' but we assume batch = 1
            x = x.permute(1,0,2,3).unsqueeze(0)
        return x
