...
from mri3d.models.vae_blocks import (
...
from mri3d.models.resnet import pseudoConv3d, tempConv1d
...
class Encoder(nn.Module):
    def __init__(
        ...
        if use_conv1d:
            self.conv_intemp = tempConv1d(in_channels, block_out_channels[0], kernel_size=3, padding=1, bias=False,)
            nn.init.zeros_(self.conv_intemp.weight.data)
    ...
    def forward(self, x):
        sample = x
        sample = self.conv_in(sample)+ self.conv_intemp(sample) if self.use_conv1d else self.conv_in(sample)
...
class Decoder(nn.Module):
    def __init__(
        ...
        if use_conv1d:
            self.conv_intemp = tempConv1d(in_channels, block_out_channels[-1], kernel_size=3, padding=1, bias=False,)
            nn.init.zeros_(self.conv_intemp.weight.data)
