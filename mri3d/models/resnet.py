...
class Downsample3D(nn.Module):
    def __init__(self, channels, use_conv=False, use_conv1d=False, out_channels=None, padding=1, name="conv", dim=0,):
        super().__init__()
        self.channels = channels
        self.out_channels = out_channels or channels
        self.use_conv = use_conv
...
class ResnetBlock3D(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.conv1 = pseudoConv3d(in_channels, out_channels, kernel_size=3, stride=1, padding=1,dim=dim)
        #TODO: Add layer
        if use_conv1d:
            self.conv11 = tempConv1d(in_channels, out_channels, kernel_size=3, stride=1, padding=1,dim=dim, bias=conv1d_bias,)
        ...
        self.conv2 = pseudoConv3d(out_channels, out_channels, kernel_size=3, stride=1, padding=1,dim=dim)
        if use_conv1d:
            self.conv21 = tempConv1d(out_channels, out_channels, kernel_size=3, stride=1, padding=1,dim=dim, bias=conv1d_bias,)
        ...
        if self.use_in_shortcut:
            self.conv_shortcut = pseudoConv3d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, dim=dim)
        self.use_conv1d = use_conv1d

        if use_conv1d:
            nn.init.zeros_(self.conv11.weight.data)
            nn.init.zeros_(self.conv21.weight.data)
    def forward(self, input_tensor, temb):
        ...
        # x = pseudo3d(x) + conv1d(x) else pseudo3d(x)
        hidden_states = self.conv11(hidden_states) + self.conv1(hidden_states) if self.use_conv1d else self.conv1(hidden_states)
        ...
        hidden_states = self.conv21(hidden_states) + self.conv2(hidden_states) if self.use_conv1d else self.conv2(hidden_states)
